...
using power_grid_model::WritableDataset;

class PgmVnfConverter {
  public:
    PgmVnfConverter(char* buffer, WritableDataset* data);

    // Public member functions
    void parse_vnf_file();
    ConstDataset* convert_input();

  private:
    // Private attributes
    char* f_file_buffer;
    WritableDataset* deserialized_data; // this type because it is generated by a deserializer type structure

    // Private setters/getters
    void set_file_buffer(char* file_buffer);

    void set_deserialized_data(WritableDataset* deserialized_data);

    char* get_file_buffer();

    WritableDataset* get_deserialized_data();
...
inline PgmVnfConverter::PgmVnfConverter(char* buffer = nullptr, WritableDataset* data = nullptr)
...
inline ConstDataset* PgmVnfConverter::convert_input() {
    convert_node_input();
    convert_line_input();
    ...
    // for now
    ConstDataset* fake_data = nullptr;
    return fake_data;
};

inline void PgmVnfConverter::set_file_buffer(char* file_buffer) { this->f_file_buffer = file_buffer; };

inline void PgmVnfConverter::set_deserialized_data(WritableDataset* deserialized_data) {
    this->deserialized_data = deserialized_data;
};

inline char* PgmVnfConverter::get_file_buffer() { return this->f_file_buffer; };

inline WritableDataset* PgmVnfConverter::get_deserialized_data() { return this->deserialized_data; };
...
inline ConstDataset* convert_input_wrapper(PgmVnfConverter* obj) { return obj->convert_input(); };
